...
function WorkBlock(props: WorkBlockProps) {
  const width = getCellWidth(props.zoom) * props.days - 9;
  return (
    <div
      onClick={props.onClick}
      id={props.id.toString()}
      style={{ width: width, height: '60px' }}
      className={`card cursor-pointer whitespace-nowrap text-xs font-semibold bg-opacity-50 bg-${props.color}-500 rounded-md flex flex-col gap-1 p-2`}>
